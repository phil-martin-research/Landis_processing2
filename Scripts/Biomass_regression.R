...
df<-data.frame(df,SRR=NA,Min_rate=NA,Fungi=NA,GF=NA,Lichen=NA,Val_fungi=NA)
Coefficients<-NULL
for (i in 4:5){
...
P1<-ggplot(BM_melt,aes(x=AGB,y=value,group=Site))+geom_point(size=3,shape=1)+facet_wrap(~variable,scales = "free")
P2<-P1+geom_line(data=df_melt,aes(group=NULL),size=2)+
...
P2+annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf)+

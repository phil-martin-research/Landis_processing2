...
for (j in 1:length(File_names)){
  ...
  species=gsub("^.*?/","",sub(".*?cohort-stats/(.*?)-MAX.*", "\\1", File_names[j])),
  scenario=sub(".*?Landis_output/(.*?)/.*", "\\1", File_names[j]),
  replicate=gsub("^.*?/","",sub(".*?run_(.*?)/output.*", "\\1", File_names[j])),
...
for (i in 1:nrow(Uniq_combs)){#for each unique combination of scenario and replicate
  ...
  Community<-data.frame(Sub_CC[,(1:3)],similarity=c(1,(1-vegdist(Sub_CC2))[1:5]))
  #bind results to last set of loop results

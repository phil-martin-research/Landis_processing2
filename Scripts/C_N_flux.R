...
for (i in 1:length(C_N)){
  ...
  File_sub3<-File_sub3[with(File_sub3, order(EcoregionIndex, Time)), ]
  for (j in 2:nrow(File_sub3)){
    head(File_sub3)
    File_sub3$C_change[j]<-File_sub3$Total_C[j]-File_sub3$Total_C[j-1]
    File_sub3$N_change[j]<-File_sub3$TotalN[j]-File_sub3$TotalN[j-1]
  }
  #bind all these outputs together
  CN_ER<-rbind(File_sub3,CN_ER)
}
...
P1<-ggplot(CN_ER_sum,aes(x=Time,y=m_c,group=EcoregionName))+geom_line(alpha=0.2)+facet_wrap(~Scenario)
...
P3+ylab(expression(paste("Carbon flux (Mg",~Ha^-1,")")))+xlab("Time (Years)")+theme(panel.margin = unit(1, "lines"))
ggsave("Figures/Carbon_flux.pdf",dpi = 400,height=6,width=8,units="in")

#make plot of nitrogen change
theme_set(theme_bw(base_size=12))
P1<-ggplot(CN_ER_sum,aes(x=Time,y=m_n,group=EcoregionName))+geom_line(alpha=0.2)+facet_wrap(~Scenario)
...
P3<-P2+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.border = element_rect(size=1.5,colour="black",fill=NA))
P3+ylab(expression(paste("Nitrogen flux")))+xlab("Time (Years)")+theme(panel.margin = unit(1, "lines"))

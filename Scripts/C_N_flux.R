...
P1<-ggplot(CN_ER_sum,aes(x=Time,y=m_c,group=EcoregionName))+geom_line(alpha=0.2)+facet_wrap(~Scenario,scales = "free_y")
...
theme_set(theme_bw(base_size=12))
P1<-ggplot(CN_ER_sum,aes(x=Time,y=m_n,group=EcoregionName))+geom_line(alpha=0.2)+facet_wrap(~Scenario,scales="free_y")
